...
const ExpandedChart: React.FC<Props> = (props) => {
  ...
  useEffect((): any => {
    ...
    const knativeServingComponent = components?.find(
      (c) =>
        c.RawYAML.apiVersion === "serving.knative.dev/v1" &&
        c.Kind === "Service"
    );
    if (
      knativeServingComponent?.RawYAML?.metadata?.annotations?.[
        "external-dns.alpha.kubernetes.io/hostname"
      ]
    ) {
      setUrl(
        `https://${knativeServingComponent.RawYAML.metadata.annotations["external-dns.alpha.kubernetes.io/hostname"]}`
      );
      return;
    }
